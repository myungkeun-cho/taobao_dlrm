...
def dlrm_wrap(X, lS_o, lS_i, use_gpu, device, ndevices=1):
    with record_function("DLRM forward"):
        ...
        return dlrm(X.to(device), lS_o, lS_i, device)
...
class my_cache:#embedding table 별로 만들어서 관리...?
    def __init__(self):
        global list_cache_access
        list_cache_access = []
        for i in range(3):#table 수?
            list_cache_access.append(dict())
        self.list_cache_access = list_cache_access

    def arrange_mini_batch(self, emb_dict_list, cache_keys, sparse_index_group_batch, sparse_offset_group_batch, hbm_keys):#table 또는 HBM, 내부 중복에 있는 key 제외시킴
        #cache_keys = cache에 있는 key들의 list
        mini_batch = dict() #{ls_o:ls_i}

        for i, off in enumerate(sparse_offset_group_batch):
            if (i + 1) == len(sparse_offset_group_batch):
                mini_batch[off] = sparse_index_group_batch[off:]
            else:
                mini_batch[off] = sparse_index_group_batch[off:sparse_offset_group_batch[i+1]]

        hbm_cache_keys = hbm_keys + cache_keys #hbm_key와 cache_key를 하나의 list로 합친 형태

        del_mini_batch = dict() #key를 제거한 mini batch를 만들기 위함

        for key, value in mini_batch.items():
            del_mini_batch[key] = list(set(value)-set(hbm_cache_keys))
            '''
            v_list = []
            for i in value:
                if i not in hbm_cache_keys:
                    v_list.append(i)
            del_mini_batch[key] = v_list
            '''
        #del_mini_batch = {ls_o:ls_i}
        #hbm_key와 cache_key를 제거한 최종 형태

        #access 횟수 기록
        num_access = dict()

        for key, value in del_mini_batch.items():
            sum_access = 0
            for i in value:
                if i in emb_dict_list:
                    sum_access += emb_dict_list[i]
                else:
                    sum_access += 1 #trainig 기준으로 0, inference 시 접근 횟수도 고려하면 1
            num_access[key] = sum_access
        #num_access = {ls_o:sum_access}

        #access 횟수가 높은 것부터 차례대로 내부적으로 중복된 key 제거
        num_list = list(num_access.items()) #[(key, value)], tuple 형태의 결과물을 list로 만듬
        s_num = sorted(num_list, key=lambda x: x[1], reverse=True)

        for key1, val1 in s_num: #key1 = ls_o, val1 = access
            for key2, val2 in del_mini_batch.items(): #key2 = ls_o, val2 = ls_i
                if key1 == key2:
                    continue
                else:
                    del_mini_batch[key2] = list(set(val2)-set(del_mini_batch[key1]))
        #del_mini_batch = 내부 중복이 제거된 상태
        #빈 list 제거
        new_mini_batch = dict()
        for i, j in del_mini_batch.items():
            if j != []:
                new_mini_batch[i] = j
        #new_mini_batch = 중복을 제거한 상태에서의 {ls_o:ls_i}

       #중복을 완전히 제거한 상태에서 access 횟수를 기록
        num_access = dict()

        for key, value in new_mini_batch.items():
            sum_access2 = 0
            for i in value:
                if i in emb_dict_list: #디버그로 값 확인해볼것.
                    sum_access2 += emb_dict_list[i]
                else:
                    sum_access2 += 1
            num_access[key] = sum_access2
        #num_access = {ls_o:access num}

        return new_mini_batch, num_access

    def cache_table(self, list_cache_access, sorted_del_cache_keys_mini_batch, access_sorted_del_cache_keys_mini_batch, embedding_table, emb_l, device, var_weight, cnt_num):#cache key 관리 및 embedding table weight 수정
        #{lS_o:lS_i} + {lS_o:access} = {lS_i:sum of access number}
        key_and_access = dict()
        for key1, val1 in sorted_del_cache_keys_mini_batch.items():
            for key2, val2 in access_sorted_del_cache_keys_mini_batch.items():
                if key1 == key2:
                    key_and_access[tuple(val1)] = val2
        #ls_i가 list이므로 tuple로 바꾼 후 key로 사용
        #key_and_access = {ls_i:access}

        #key, weight updates
        if not list_cache_access:#cold start
            list_cache_access = key_and_access
            for key, val in list_cache_access.items(): #key = ls_i, val = access
                input = torch.tensor(key).to(device) #ls_i
                offset = torch.tensor([0]).to(device)
                mean = emb_l(input, offset) / len(key) #weight의 평균
                for i in key:
                    if cnt_num % 3 == 0: #table 수?
                        var_weight.append(embedding_table.weight[i] - mean)
                    embedding_table.weight[i] = mean #weight 평균값으로 embedding_table.weight update
        else: #access를 기준으로 key 업데이트 필요.
            for key1, val1 in key_and_access.items(): #새로 들어온 {ls_i:access}
                if len(list_cache_access.values()) < 12: #cache_size, 현재 hbm store = 250, cache =125
                    list_cache_access[key1] = val1
                    input = torch.tensor(key1).to(device)
                    offset = torch.tensor([0]).to(device)
                    mean = emb_l(input, offset) / len(key1)
                    for i in key1:
                        if cnt_num % 3 == 0: #table 수?
                            var_weight.append(embedding_table.weight[i] - mean)
                        embedding_table.weight[i] = mean
                else:
                    for key2, val2 in list_cache_access.items(): #기존 {ls_i:access}
                        if val1 > val2:#access 횟수가 새로 들어온게 더 높으면 update
                            #list_cache_access[key1] = list_cache_access.pop(key2)
                            list_cache_access[key1] = val1 #새로 key, access 기록 update
                            for j in key2:#제거된 group은 DIMM에서 data를 가져와야되므로 원본 weight로 복구
                                embedding_table.weight[j] = emb_l.weight[j]
                            del list_cache_access[key2] #기존 group 제거
                            input = torch.tensor(key1).to(device)
                            offset = torch.tensor([0]).to(device)
                            mean = emb_l(input, offset) / len(key1)
                            for i in key1:
        ...
        cache_keys = []
        for tuple_keys in list_cache_access.keys():
            cache_keys.extend(list(tuple_keys))

        return cache_keys #최종적으로 cache에 있는 key들의 list
...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        ...
        #copy embedding table
        for i, embedding_l in enumerate(emb_l):
            Emb = nn.EmbeddingBag.from_pretrained(embedding_l.weight, mode = 'sum')
            Emb.weight.requires_grad = False
            embedding_table.append(Emb)

        return emb_l, v_W_l, embedding_table
    ...
    def __init__(
        ...
        if (
            ...
            # create operators
            if ndevices <= 1:
                self.emb_l, w_list, self.embedding_table = self.create_emb(m_spa, ln_emb, weighted_pooling)
    ...
    def apply_emb(self, lS_o, lS_i, v_W_l, embedding_table, emb_l, emb_dict_list, list_cache_access, device, hbm_keys):
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            if self.quantize_emb:
                ...
                if args.inference_only and args.my_cache: #cache
                    E = embedding_table[k]
                    V = E(sparse_index_group_batch, sparse_offset_group_batch, per_sample_weights=per_sample_weights)

                    global cnt_num
                    global sdckmb #new_mini_batch -> {ls_o:ls_i}
                    global asdckmb #num_access -> {ls_o:access}
                    global c_keys
                    global var_weight
                    if cnt_num < 3: #table 수?
                        sdckmb, asdckmb = custom_cache.arrange_mini_batch(emb_dict_list[k], [], sparse_index_group_batch, sparse_offset_group_batch, hbm_keys[k])
                        c_keys.append(custom_cache.cache_table(list_cache_access[k], sdckmb, asdckmb, E, emb_l[k], device, var_weight, cnt_num))
                        cnt_num = cnt_num + 1
                    else:
                        sdckmb, asdckmb = custom_cache.arrange_mini_batch(emb_dict_list[k], c_keys[k], sparse_index_group_batch, sparse_offset_group_batch, hbm_keys[k])
                        c_keys[k] = custom_cache.cache_table(list_cache_access[k], sdckmb, asdckmb, E, emb_l[k], device, var_weight, cnt_num)
                        cnt_num = cnt_num + 1
                elif not args.inference_only or not args.my_cache:
                    global_dict[k] = Counter(global_dict[k]) + Counter(sparse_index_group_batch.tolist())
                    E = emb_l[k]
                    V = E(
                        sparse_index_group_batch,
                        sparse_offset_group_batch,
                ...
                ly.append(V)
            if args.my_cache:
                with open("0930_var_weight_test.txt", "a") as file:
                    file.write("\nvar_weight\n")
                    file.write(str(var_weight))
                    file.write("\n")
            var_weight = []
        # print(ly)
        return ly
    ...
    def forward(self, dense_x, lS_o, lS_i, device):
        if ext_dist.my_size > 1:
            # multi-node multi-device run
            return self.distributed_forward(dense_x, lS_o, lS_i)
        elif self.ndevices <= 1:
            # single device run
            return self.sequential_forward(dense_x, lS_o, lS_i, device)
        else:
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i, device):
        ...
        ly = self.apply_emb(lS_o, lS_i, self.v_W_l, embedding_table, self.emb_l, emb_dict_list, list_cache_access, device, hbm_keys)
        # for y in ly:
    ...
    def parallel_forward(self, dense_x, lS_o, lS_i):
        ...
        # embeddings
        ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l, self.embedding_table)
        # debug prints
        # print(ly)
...
def inference(
    ...
    for i, testBatch in enumerate(test_ld):
        ...
        #T_test = np.round(T_test, 0) #MG

        #with open("0930_wo_cache_test.txt","a") as file:
        #            file.write("lS_i_test\n")
        #            for i in lS_i_test:
        #                file.write(str(i))
        #            file.write("\n")
        #
        ...
        if args.mlperf_logging:
            ...
            with record_function("DLRM accuracy compute"):
                ...
                S_test = Z_test.detach().cpu().numpy()  # numpy array
                #with open("0930_w_cache_test.txt","a") as file:
                #    file.write("\nS_test\n")
                #    for i in S_test:
                #        file.write(str(i))
                #    file.write("\n")
                ...
                #    if np.round(S_test[i][0], 0) == T_test[i][0]:
                #        A_test += 1
                #    else:
                #        A_test += 0
...
def run():
    ...
    # my cache
    parser.add_argument("--my-cache", action="store_true", default=False)
    # quantize
    ...
    #training
    global global_dict
    global_dict = [[] for _ in range(3)]#np.fromstring(args.arch_embedding_size, dtype=int, sep="-").size)]

    #inference
    global emb_dict_list
    emb_dict_list = []

    #read file for access_num
    if args.inference_only:
        with open('./0930_test_access_number.log', 'r') as file_data:
            for i, line in enumerate(file_data):
                if i<8:
                    ...
                    for data in line_s:
                        v_and_k = data.split(':')
                        k_list.append(str(int(v_and_k[0])))
                        v_list.append(int(v_and_k[1]))
                    my_dict = dict(zip(k_list, v_list))
                    emb_dict_list.append(my_dict)

    #hbm에 offloading 되는 key 설정
    global hbm_keys
    hbm_keys = []

    if args.inference_only:
        for a, i in enumerate(emb_dict_list):
            hbm_keys.append([])
            access_keys = list(map(int,i.keys()))
            #access_keys_index = round(len(access_keys)*0.1)
            access_keys_index = 3
            hbm_keys[a].extend(access_keys[0:access_keys_index])
    ...
    # test prints
    #if args.debug_mode or not my_cache:
    if False:#1022
        with open("1007_kaggle_initial_weight.txt","a") as file:
            file.write("\ninitial parameters (weights and bias):\n")
            for param in dlrm.parameters():
                print("******************")
                for i in param.detach().cpu().numpy():
                    file.write(str(i))
    ...
    if use_gpu:
        ...
        if dlrm.ndevices > 1:
            dlrm.emb_l, dlrm.v_W_l, dlrm.embedding_table = dlrm.create_emb(
        ...
        if not args.inference_only:
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    should_test = (
                        (args.test_freq > 0)
                        and (args.data_generation in ["dataset", "random", "taobao"])
    ...
    #if not args.inference_only:
    #    with open("./1007_kaggle_test_access_number.log", "w") as f:
    #        for i in range(ln_emb.size):
    #            f.write(str(global_dict[i]).replace('Counter({','').replace('})','') + '\n')
    ...
    if False:#1022
        with open("1007_kaggle_updated_weight.txt","a") as file:
